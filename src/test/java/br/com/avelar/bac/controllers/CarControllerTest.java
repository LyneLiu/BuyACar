...
public class CarControllerTest {
    ...
    public void testFindByLicense() throws Exception {
        ...
        mock.perform(get("/car/license/GCA-3294"))
                ...
                .andExpect(
                        jsonPath("$.mileage", is(car.getMileage().intValue())))
                ...
                .andExpect(
                        jsonPath("$.model.manufacturer.name", is(car.getModel()
                                .getManufacturer().getName())))
                .andExpect(
                        jsonPath("$.model.name", is(car.getModel().getName())));
    }
    ...
    public void testFindByDescription() throws Exception {
        ...
        mock.perform(get("/car/description/BMW"))
                ...
                .andExpect(
                        jsonPath("$[0].description", is(car.getDescription())))
                .andExpect(
                        jsonPath("$[0].mileage",
                                is(car.getMileage().intValue())))
                .andExpect(
                        jsonPath("$[0].combustible", is(car.getCombustible())))
                .andExpect(jsonPath("$[0].color", is(car.getColor())))
                .andExpect(
                        jsonPath("$[0].doorsNumber", is(car.getDoorsNumber())))
                .andExpect(jsonPath("$[0].year", is(car.getYear())))
                .andExpect(jsonPath("$[0].price", is(car.getPrice())))
                .andExpect(
                        jsonPath("$[0].model.manufacturer.name", is(car
                                .getModel().getManufacturer().getName())))
                .andExpect(
                        jsonPath("$[0].model.name",
